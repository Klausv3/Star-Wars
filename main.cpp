...
/* vordefiniert in person.h
enum class Planet{Kashyyyk, Coruscant, Corellia, Vulcan};
const std::vector<std::string> planet_names{"Kashyyyk", "Coruscant", "Corellia", "Vulcan"};
*/
...
int main(){
  ...
  cout << testSpaceship << '\n';
  vector<Person> exp = testSpaceship.expedition();
  for (const auto& p : exp)
	  cout << p << '\n';
  testSpaceship.hire({"Test1",70});
  testSpaceship.hire({"Test2",70,Planet::Corellia});
  testSpaceship.hire({"Test3",74,Planet::Corellia});
  exp = testSpaceship.expedition();
  for (const auto& p : exp)
	  cout << p << '\n';
  cout << testSpaceship << "\n";
  testSpaceship.hire({"Test4",74,Planet::Vulcan});
  ...
  exp = millenniumFalcon2.expedition();
  for (const auto& p : exp)
	  cout << p << '\n';
  ...
  */
  ...
  Spaceship millenniumFalcon3 {"MillenniumFalcon3", {han, chewie, lando, wedge}};
  const vector<Spaceship> schiffe{millenniumFalcon, constTestSpaceship, Spaceship{"Last", {leia, wedge}}, millenniumFalcon3};
  cout << Spaceship{"A-Team",Spaceship::check_fleet(schiffe),Planet::Coruscant} << '\n';

  const Person ackbar{"Admiral Ackbar",100};
  const Person tarkin{"Willhuff Tarkin",100};
  const Person krennic{"Orson Krennic",80};
  const Person karrde{"Talon Karrde",100};

  cout << Spaceship{"MC-80",Spaceship::check_fleet({Spaceship{"CR-90",{ackbar,tarkin}},Spaceship{"CR-90",{tarkin,karrde}},Spaceship{"CR-90",{krennic,karrde,ackbar}},Spaceship{"CR-90",{karrde,tarkin}}})} << '\n';
  */
